...
Capistrano::Configuration.instance(:must_exist).load do |configuration|
  ...
  set :harp_port, "5000" unless defined? harp_port
  set :harp_binary, "/usr/bin/harp" unless defined? harp_binary
...
script
    cd #{current_path} && exec sudo -u #{node_user} NODE_ENV=#{node_env} #{app_environment} #{harp_binary} server #{current_path}/ --port #{harp_port} 2>> #{stderr_log_path} 1>> #{stdout_log_path}
end script
EOD
  }
